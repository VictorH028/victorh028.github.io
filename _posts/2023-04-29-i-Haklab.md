...
Con esto podríamos usar la tecla `CTRL + s` como un prefijo para nuestros atajos.

```shell
vim.keymap.set('n', '<leader>w', '<cmd>write<cr>')
```
Y así la secuencia `CTRL + s + w` guarda el archivo actual.
...
😎 La configuracion de cada plugins que acabas de ver  esta en un directorio independiente haci como las combinaciones de teclas para un mejor itendimiento.
